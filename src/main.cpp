...
void loop()
{
  ...
  if (last_encoders.left_encoder <= threshold
      && encoders.left_encoder >= 4095 - threshold) {
    encoders.left_nb_revolution--;
  }
  else if (encoders.left_encoder <= threshold
      && last_encoders.left_encoder >= 4095 - threshold) {
    encoders.left_nb_revolution++;
  }

  if (last_encoders.right_encoder <= threshold
      && encoders.right_encoder >= 4095 - threshold) {
    encoders.right_nb_revolution--;
  }
  else if (encoders.right_encoder <= threshold
      && last_encoders.right_encoder >= 4095 - threshold) {
    encoders.right_nb_revolution++;
  }
